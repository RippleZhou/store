// pages/activity0903/activity0903.js
...
Page({
  data: {
    ...
    stateShow: 0,
    activityState: null, //1 开始 0 活动未开始 -1 结束
    isApp: false,
    beanAmount: 0,
    activityInfo: {},
    list: [],
  ...
  // 立即使用
  async joinActivity() {
    console.log('joinActivity', joinActivity)
    let {
      activityId
    } = this.data
    ...
    console.log('joinActivity', customerCode)
    const self = this;
    // if (!Common.biz.loggedIn('/activity0903')) return;
    let url = `${Api.userEnvelope.takeActivityEnv}`
    ...
    try {
      ...
      if (err.message == '您已经领取过了') {
        wx.showModal({
          content: err.message,
          showCancel: false
        })
        wx.setStorage({
          key: 'popupActivity0903',
          data: customerCode
        })
      }
    }
  ...
  openPopup() {
    ...
    Common.request.post(url, reqParams, function (data) {
      ...
      let envelopeId = data.message.envEnvelopeId != null ? data.message.envEnvelopeId : 0
      let stateShow = 2
      that.setData({
        bRotateC,
        beanBag,
  ...
  async getStatus() {
    ...
    let uid = Common.getCustomerCode()
    console.log('uiduiduiduid:',uid)
    let url = `${Api.userEnvelope.checkCollarBag}?customerCode=${uid}`
    ...
    try {
      ...
      if (res.message.remark == '可以抢购') {
        this.setData({
          envEnvelopeId: res.message.envEnvelopeId,
          status: res.status,
          isShow: true,
          beanHidden: true
        })
      ...
      if (code == 101) {
        wx.showModal({
          content: err.message.remark,
          showCancel: false,
          success() {
            wx.navigateTo({
              url: `/pages/beanBagDetail/beanBagDetail?envEnvelopeId=${err.message.envEnvelopeId}`
            })
          }
        })
      }
  ...
  chaiBean() {
    console.log('chaiBean')
    this.getStatus()
    // .then((data) => {
    //   //  当前可以抢    status:0
    //   //  当前不可以抢  status:1
    //   that.envEnvelopeId = data.envEnvelopeId
    //   that.status = data.status
    //   that.isShow = true;
    // });

    function check() {
      var token = that.$comjs.getToken()
      if (token && token.id) return true;
      else {
        that.$router.push('/Login') //跳转登录
        return false
      }
    }

    //       error => {
    //         if (error instanceof that.$common.error.api) {
    //           switch (error.message.data.code) {
    //             case 100://抢完
    //               resolve({
    //                 status: 1,
    //                 envEnvelopeId: error.message.data.message.envEnvelopeId
    //               })
    //               break;
    //             case 101://抢过
    //               that.$vux.toast.show({
    //                 text: error.message.data.message.remark,
    //                 type: 'text',
    //                 onHide: () => {
    //                   that.$router.push({ name: 'beanBagDetail', params: { envEnvelopeId: error.message.data.message.envEnvelopeId } });
    //                 }
    //               })
    //               break;
    //             default:
  ...
  setActivityTime: function (activityInfo) {
    console.log('setActivityTime')
    let now = new Date();

    let EndTime = activityInfo.activeEndTime.replace(/-/g, '/')
    let StartTime = activityInfo.activeStartTime.replace(/-/g, '/')

    let activeEndTime = new Date(EndTime);
    let activeStartTime = new Date(StartTime);
    ...
    if (activeStartTime.getTime() > now.getTime()) {
      this.setData({
        activityState: 0 //"活动未开始"
      })
    } else if (activeEndTime.getTime() < now.getTime()) {
      this.setData({
        activityState: -1 //"活动已经结束"
      })
    } else {
      this.setData({
        activityState: 1,
      })
      this.countDown(now, activeEndTime);
    }
  },
  countDown: function (startDate, endDate) {
    ...
    if (hours >= 0) {
      ...
      self.timer = setTimeout(function () {
        let now = new Date();
        self.countDown(now, endDate);
      }, 1000)
    }
  },
 async getRedEnvelope() {
   console.log('getRedEnvelope:::')
   let customerCode = Common.getCustomerCode()
   var user = await Common.getUser(customerCode)
  ...
  async getActivityInfo() {
    let url = `${Api.queryActivity}?offset=1&limit=2`
    ...
    if (res.rows.length > 0) {
      ...
      this.setActivityTime(res.rows[0]);
